...
installmod() {
  ...
  # Apply USB fix
  if [ "$(find $MODPATH/system -type f -name "usb_audio_policy_configuration.xml" 2>/dev/null)" ]; then
    for FILE in $(find $MODPATH/system -type f -name "usb_audio_policy_configuration.xml" 2>/dev/null); do
    ...
    for FILE in $(find $MODPATH/system -type f -name "*audio_*policy*.conf" 2>/dev/null); do
